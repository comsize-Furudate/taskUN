...
<body>
	...
	<p style="text-align: right">
		...
		ユーザーID：<%=session.getAttribute("id")%><br> ユーザー名：<%=session.getAttribute("name")%>さん<br>
 ...
 %>
	...
	%>
	<%=bean.getUserId()%><%=bean.getComment()%><%=bean.getUpdateDateTime()%>
	<%
	if (session.getAttribute("id").equals(bean.getUserId())) {
	%>
	<a href="comment-delete-check-servlet?comment_id=<%=bean.getCommentId()%>">削除</a><br>
	...
	<form action="comment-post-servlet" method="post">
		<textarea name="commentPost" rows="4" cols="25" maxlength="100">コメントを投稿する。</textarea>
